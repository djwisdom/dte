...
static void test_buf_parse_hex_uint(TestContext *ctx)
{
    ...
    val = 0x90;
    EXPECT_EQ(buf_parse_hex_uint(strview(buf), &val), buf_len);
    EXPECT_UINT_EQ(val, UINT_MAX);

    buf_len = xsnprintf(buf, sizeof buf, "1%x", UINT_MAX);
    val = 0x100;
    EXPECT_EQ(buf_parse_hex_uint(strview(buf), &val), 0);
    EXPECT_UINT_EQ(val, 0x100);

    buf_len = xsnprintf(buf, sizeof buf, "%xg", UINT_MAX);
    val = 0x110;
    EXPECT_EQ(buf_parse_hex_uint(strview(buf), &val), buf_len - 1);
    EXPECT_UINT_EQ(val, UINT_MAX);
    ...
    val = 0x130;
    EXPECT_EQ(buf_parse_hex_uint(string_view(buf, buf_len), &val), buf_len);
    EXPECT_UINT_EQ(val, 0);

    val = 0x140;
    EXPECT_EQ(buf_parse_hex_uint(string_view(buf, buf_len - 2), &val), buf_len - 2);
    EXPECT_UINT_EQ(val, 0);

    val = 0x150;
    EXPECT_EQ(buf_parse_hex_uint(strview("12345678"), &val), 8);
    EXPECT_UINT_EQ(val, 0x12345678);

    val = 0x160;
    EXPECT_EQ(buf_parse_hex_uint(strview("00abcdeF01"), &val), 10);
    EXPECT_UINT_EQ(val, 0xABCDEF01);

    val = 0x170;
    EXPECT_EQ(buf_parse_hex_uint(string_view("0123456789", 4), &val), 4);
...
static void test_u_make_printable(TestContext *ctx)
{
    ...
    EXPECT_EQ(u_make_printable(strview("\xffxyz"), buf, sizeof(buf), flags), 4);
    EXPECT_STREQ(buf, "<ff>");
    ...
    EXPECT_EQ(u_make_printable(strview("12345"), buf, 1, flags), 0);
    EXPECT_EQ(u_make_printable(strview("12345"), buf, 2, flags), 0);
    EXPECT_EQ(u_make_printable(strview("12345"), buf, 3, flags), 0);
    EXPECT_EQ(u_make_printable(strview("12345"), buf, 4, flags), 0);
    EXPECT_EQ(buf[0], '\0');
    EXPECT_EQ(buf[1], '_');
    EXPECT_EQ(buf[2], '_');
    EXPECT_EQ(buf[3], '_');
    EXPECT_EQ(buf[4], '_');
    memset(buf, '_', sizeof(buf));
    EXPECT_EQ(u_make_printable(strview("12345"), buf, 5, flags), 1);
    EXPECT_EQ(buf[0], '1');
    EXPECT_EQ(buf[1], '\0');
    EXPECT_EQ(buf[2], '_');
    EXPECT_EQ(buf[3], '_');
    ...
    memset(buf, '_', sizeof(buf));
    EXPECT_EQ(u_make_printable(strview("\x7F\n123"), buf, 5, flags), 2);
    EXPECT_STREQ(buf, "^?");
    EXPECT_EQ(buf[3], '_');
    ...
    memset(buf, '_', sizeof(buf));
    EXPECT_EQ(u_make_printable(string_view(STRN("\0xyz")), buf, 5, flags), 3);
