...
static void expect_parse_seq (
    ...
    static_assert(TPARSE_NO_MATCH == 0);
    if (expected_length <= TPARSE_NO_MATCH) {
        test_pass(ctx);
        return;
    }
    ...
    for (size_t n = expected_length - 1; n != 0; n--) {
        ...
        test_fail (
            ctx, file, line,
            "Parsing truncated sequence of %zu bytes returned %zd; expected %zd",
            n, parsed_length, (ssize_t)TPARSE_PARTIAL_MATCH
        );
...
static void test_term_style_to_string(TestContext *ctx)
{
    static const struct {
        ...
        {"red yellow bold", {.fg = COLOR_RED, .bg = COLOR_YELLOW, .attr = ATTR_BOLD}},
        {"#ff0000", {.fg = COLOR_RGB(0xff0000), .bg = -1}},
        {"#f00a9c reverse", {.fg = COLOR_RGB(0xf00a9c), .bg = -1, .attr = ATTR_REVERSE}},
        {"black #00ffff", {.fg = COLOR_BLACK, .bg = COLOR_RGB(0x00ffff)}},
        {"#010900", {.fg = COLOR_RGB(0x010900), .bg = COLOR_DEFAULT}},
        {"red strikethrough", {.fg = COLOR_RED, .bg = -1, .attr = ATTR_STRIKETHROUGH}},
        {"231", {.fg = 231, .bg = COLOR_DEFAULT}},
        {"70 48 italic", {.fg = 70, .bg = 48, .attr = ATTR_ITALIC}},
        {"default keep", {.fg = COLOR_DEFAULT, .bg = COLOR_KEEP}},
        {"keep red keep", {.fg = COLOR_KEEP, .bg = COLOR_RED, .attr = ATTR_KEEP}},
        {"88 16 blink bold", {.fg = 88, .bg = 16, .attr = ATTR_BOLD | ATTR_BLINK}},
        {"black 255 underline", {.fg = COLOR_BLACK, .bg = 255, .attr = ATTR_UNDERLINE}},
        {"white green dim", {.fg = COLOR_WHITE, .bg = COLOR_GREEN, .attr = ATTR_DIM}},
        {"darkgray lightgreen", {.fg = COLOR_DARKGRAY, .bg = COLOR_LIGHTGREEN}},
        {"lightmagenta", {.fg = COLOR_LIGHTMAGENTA, .bg = COLOR_DEFAULT}},
        {"keep 254 keep", {.fg = COLOR_KEEP, .bg = 254, .attr = ATTR_KEEP}},
        {"default", {.fg = COLOR_DEFAULT, .bg = COLOR_DEFAULT}},
        {"default bold", {.fg = COLOR_DEFAULT, .bg = COLOR_DEFAULT, .attr = ATTR_BOLD}},
        {"default default keep", {.fg = COLOR_DEFAULT, .bg = COLOR_DEFAULT, .attr = ATTR_KEEP}},
    };
...
{
    EXPECT_PARSE_SEQN("", TPARSE_PARTIAL_MATCH, 0);
    EXPECT_PARSE_SEQN("x", TPARSE_NO_MATCH, 0);
    EXPECT_PARSE_SEQN("\033q", TPARSE_NO_MATCH, 0);
    ...
    EXPECT_PARSE_SEQ("\033[57384u", KEY_F21);
    EXPECT_PARSE_SEQ("\033[57385u", KEY_F22);
    EXPECT_PARSE_SEQ("\033[57386u", KEY_F23);
    EXPECT_PARSE_SEQ("\033[57387u", KEY_F24);
    EXPECT_PARSE_SEQ("\033[57388u", KEY_F25);
    EXPECT_PARSE_SEQ("\033[57389u", KEY_F26);
    EXPECT_PARSE_SEQ("\033[57390u", KEY_F27);
    EXPECT_PARSE_SEQ("\033[57391u", KEY_F28);
    EXPECT_PARSE_SEQ("\033[57392u", KEY_F29);
    EXPECT_PARSE_SEQ("\033[57393u", KEY_F30);
    EXPECT_PARSE_SEQ("\033[57394u", KEY_F31);
    EXPECT_PARSE_SEQ("\033[57395u", KEY_F32);
    EXPECT_PARSE_SEQ("\033[57396u", KEY_F33);
    EXPECT_PARSE_SEQ("\033[57397u", KEY_F34);
    EXPECT_PARSE_SEQ("\033[57398u", KEY_F35);
    ...
    EXPECT_PARSE_SEQ("\033[57416u", KEY_IGNORE); // KP_SEPARATOR
...
static void test_keycode_to_string(TestContext *ctx)
{
    static const struct {
        ...
        {"C-S-F24", MOD_CTRL | MOD_SHIFT | KEY_F24},
        {"C-S-F35", MOD_CTRL | MOD_SHIFT | KEY_F35},
...
static void test_parse_key_string(TestContext *ctx)
{
    ...
    EXPECT_EQ(keycode_from_str("C-M-S-F24"), MOD_CTRL | MOD_META | MOD_SHIFT | KEY_F24);
    EXPECT_EQ(keycode_from_str("C-M-S-F35"), MOD_CTRL | MOD_META | MOD_SHIFT | KEY_F35);
    ...
    EXPECT_EQ(keycode_from_str("f21"), KEY_F21);
    EXPECT_EQ(keycode_from_str("f22"), KEY_F22);
    EXPECT_EQ(keycode_from_str("f23"), KEY_F23);
    EXPECT_EQ(keycode_from_str("f24"), KEY_F24);
    EXPECT_EQ(keycode_from_str("f25"), KEY_F25);
    EXPECT_EQ(keycode_from_str("f26"), KEY_F26);
    EXPECT_EQ(keycode_from_str("f27"), KEY_F27);
    EXPECT_EQ(keycode_from_str("f28"), KEY_F28);
    EXPECT_EQ(keycode_from_str("f29"), KEY_F29);
    EXPECT_EQ(keycode_from_str("f30"), KEY_F30);
    EXPECT_EQ(keycode_from_str("f31"), KEY_F31);
    EXPECT_EQ(keycode_from_str("f32"), KEY_F32);
    EXPECT_EQ(keycode_from_str("f33"), KEY_F33);
    EXPECT_EQ(keycode_from_str("f34"), KEY_F34);
    EXPECT_EQ(keycode_from_str("f35"), KEY_F35);
    ...
    EXPECT_EQ(keycode_from_str("F36"), KEY_NONE);
